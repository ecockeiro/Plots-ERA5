...
file_0 = xr.open_dataset(
    '/home/coqueiro/ufrj/fis_nuvens/dados/01_07_19_oceanico_multi.nc',
    decode_times = False
    )
...
file_2 = xr.open_dataset(
    '/home/coqueiro/ufrj/fis_nuvens/dados/01_07_19_oceanico_single.nc'
    ).metpy.parse_cf()
...
for i in range(len(file_1.variables['time'])):
    ...
    q = file_1.q.metpy.sel(
        time = file_1.time[i] ,  vertical=level, latitude=lat_slice, longitude=lon_slice).metpy.unit_array.squeeze()*1000
    ...
    intervalo_min3 = np.amin(np.array(t))
    intervalo_max3 = np.amax(np.array(t))
    ...
    #--------------------------------------------------------------------------
    # Salva imagem
    plt.savefig(f'temperatura_umidade_{date_formatted}.png', bbox_inches='tight')